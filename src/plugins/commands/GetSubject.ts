...
export default class extends BaseCommand {
    ...
    async execute(context: MessageContext, args: string[], next: any) {
        if (args.length > 0) {
            ...
            const message = [];
            message.push(`🔸 ${subject.name} (#${subject.subjectId})`, `▪ Преподаватель: ${subject.teacher}`, `▪ Кабинет: ${subject.location}`, ``);
            ...
            if (subject.payload.schedules.length > 0) {
                ...
                for (const item of sortedSchedules) {
                    ...
                    subjectSchedules.push(`${moment(`${schedule.subjectDay} ${schedule.payload.time.timeStarts}`, 'd HH:mm').format('dddd в HH:mm')} (${schedule.isEven ? 'четная' : 'нечетная'}|#${schedule.scheduleId})`);
                }
            ...
            if (homeworkNumbers.length > 0) {
                message.push(`\n⚠ Номера домашних заданий по предмету: #${homeworkNumbers.join(', #')}`, `Для просмотра домашнего задания используйте /дз <номер_задания>`);
            }

            return context.send(message.join('\n'));
        } else {
            await context.reply('⚠ Вы не указали название или идентификатор предмета');
        }
    }
}
