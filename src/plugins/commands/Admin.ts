        ...
        if (args.length > 0) {
            switch (args[0]) {
                case 'предметы': {
                    ...
                    context.send(`Список предметов (сорт. 0-9)`);

                    for (const subject of subjectList) {
                        const message = [];
                        message.push(
                            `🔸 ${subject.name} (id: ${subject.subjectId})`,
                            `место: ${subject.location}`,
                            `учитель: ${subject.teacher}`,
                            `уроки: ${subject.payload.homeworks.length > 0 ? 'yes' : 'no'}\n`
                        );
                ...
                case 'расписание': {
                    const scheduleList = await schedules.find().sort({ scheduleId: 1 })
                        .populate('payload.subject payload.replacement payload.time').exec();
                    context.send(`Список расписаний (сорт. 0-9)`);

                    for (const schedule of scheduleList) {
                        const message = [];
                        message.push(
                            `🔸 ${schedule.payload.subject.name} (# распис.: ${schedule.scheduleId})`,
                            `день предмета: ${schedule.subjectDay}`,
                            `четный?: ${schedule.isEven}`,
                            `время: ${schedule.payload.time.timeId} (${schedule.payload.time.timeStarts}-${schedule.payload.time.timeEnds})`
                        )
